...
from self_paced_ensemble.canonical_ensemble.canonical_ensemble import SMOTEBoostClassifier, SMOTEBaggingClassifier, \
    RUSBoostClassifier, RandomUnderSampler, UnderBaggingClassifier,BalanceCascadeClassifier
...
def svm_HD(end_data, test,num,train):
    ...
    for iter in range(len(end_data)):
        ...
        classification = list()
        for i in range(num):
            classification.append(SVC(probability = True))
        ...
        met = []
        under = UnderBaggingClassifier(base_estimator = SVC())
        ...
        met = []
        bcc = BalanceCascadeClassifier(base_estimator=SVC(probability=True))
        ...
        # ===========================================BaggingClassifier======================
        met = []
        bc = BaggingClassifier(base_estimator=SVC(probability=True))
        ...
        met =[]
        smb =SelfPacedEnsembleClassifier(base_estimator = SVC(probability = True))
        ...
        #===========================================ResampleClassifier======================
        met =[]
        rc = ResampleClassifier(base_estimator = SVC(probability = True))
        ...
        #===========================================RUSBoostClassifier======================
        met = []
        rusb = RUSBoostClassifier(base_estimator = SVC(probability = True))
        ...
        met = []
        smo = SMOTEBaggingClassifier(base_estimator = SVC(probability = True))
    ...
    # smob = SMOTEBoostClassifier(base_estimator = SVC(probability = True))
    # smob.fit(train[iter].iloc[:,:-1].values,train[iter].iloc[:,-1].values)
    # pre = smob.predict(test[iter].iloc[:,:-1].values)
    # accuracy = metrics.accuracy_score(test[iter].iloc[:,-1].values,pre)
    # f1 = metrics.f1_score(test[iter].iloc[:,-1].values,pre)
    # roc = roc_auc_score(test[iter].iloc[:,-1].values,pre)
    # mcc = matthews_corrcoef(test[iter].iloc[:,-1].values,pre)
    # kappa = cohen_kappa_score(test[iter].iloc[:,-1].values,pre)
    # gmean = Gmean(test[iter].iloc[:,-1].values,pre)
    ...
    # eec = EasyEnsembleClassifier(base_estimator=SVC())
    # eec.fit(train[iter].iloc[:, :-1].values, train[iter].iloc[:, -1].values)
    # pre = eec.predict(test[iter].iloc[:, :-1].values)
    # accuracy = metrics.accuracy_score(test[iter].iloc[:, -1].values, pre)
    # f1 = metrics.f1_score(test[iter].iloc[:, -1].values, pre)
    # roc = roc_auc_score(test[iter].iloc[:, -1].values, pre)
    # mcc = matthews_corrcoef(test[iter].iloc[:, -1].values, pre)
